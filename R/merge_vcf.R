...
vcf_to_table <- function(x) {
    ...
    out <- cbind(
        as.data.frame(rowRanges(x)),
        as.data.frame(fixed(x)),
        as.data.frame(info(x))
    )

    out$ALT <- unlist(lapply(out$ALT, function(x) {
        return(paste(as.character(x), collapse = ","))
    }))

    out$ANN <- unlist(lapply(out$ANN, function(x) {
        return(x[1])
    }))
...
correct_Orf1ab_gene <- function(x, genes) {
    ...
    for (i in seq_len(nrow(genes))) {
        ...
        who <- which(
            x$POS >= genes[i, ]$start_pos & x$POS <= genes[i, ]$end_pos
        )
...
read_gene_coordinates <- function(gff_path) {
    ...
    gene_annot[which(
        gene_annot$gene_name %in% paste0("NSP", seq_len(10))
    ), ]$gene_name <- "ORF1a"

    gene_annot[which(
        str_detect(gene_annot$gene_name, "NSP")
    ), ]$gene_name <- "ORF1b"
